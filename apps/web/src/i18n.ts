...
const zhCN: AppCopy = {
  ...
  agent: {
    ...
    selectedFallbackSuggestions: ["解释选中内容", "生成简洁学习笔记", "查找缺失假设"],
    pageSuggestions: (title, concept) => [`解释“${title}”`, "生成本页考试风格笔记", `根据“${concept}”考我`, "查找缺失假设"],
...
const enUS: AppCopy = {
  ...
  agent: {
    ...
    selectedFallbackSuggestions: ["Explain selected content", "Create concise study notes", "Find missing assumptions"],
    pageSuggestions: (title, concept) => [`Explain "${title}"`, "Create exam-style notes for this page", `Quiz me on "${concept}"`, "Find missing assumptions"],
